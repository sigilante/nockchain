...
/// Bincode tag bytes for the post-MoE `PlainProof` layout. Pearl certificate V3
/// retains this payload layout; V3 changes the work statement, not this proof
/// serialization.
...
/// for a dense share. The production [`PearlPlainProof::from_attempt`]
/// constructor builds dense Gateway shares; Nockchain MoE admission uses the
/// separate compact recursive artifact.
...
pub struct PearlPlainProof {
    ...
    /// MoE tail. `None` for a dense share.
...
impl PearlPlainProof {
    ...
    /// Serialize exactly as Pearl's post-MoE `PlainProof.to_base64()` layout:
    ...
    /// Encode the only accepted post-MoE `PlainProof` layout.
    pub fn encode_bincode1(&self, out: &mut Vec<u8>) -> Result<(), PearlPlainProofError> {
        ...
        match &self.moe {
            ...
            None => out.push(BINCODE_OPTION_NONE_TAG),
        }
        Ok(())
    }
}
...
mod tests {
    ...
    /// Serde mirror of Pearl's post-MoE `PlainProof`, serialized by Pearl's
    /// bincode-1 crate as the byte oracle for the hand-rolled encoder.
    ...
    fn build_dense_proof() -> PearlPlainProof {
        let params = test_params();
        let config = test_config();
        let (a, b) = synth_matrices(b"pearl-plain-proof-v3", &params);
        let attempt = evaluate_pearl_merge_ticket_attempt(
            &test_header(),
            &config,
            &params,
    ...
    /// The V3-compatible dense payload strictly deserializes under Pearl's exact
    /// bincode-1 fixint layout with no MoE tail.
    #[test]
    fn pearl_v3_dense_bytes_deserialize_as_moe_none() {
        let proof = build_dense_proof();
        let expected = SerdePlainProof::from(&proof);
        assert!(expected.moe.is_none());

        let mut encoded = Vec::new();
        proof.encode_bincode1(&mut encoded).unwrap();
        let decoded: SerdePlainProof =
            bincode1::deserialize(&encoded).expect("strict bincode-1 decode");
        assert_eq!(decoded, expected);
        assert!(decoded.moe.is_none());
        assert_eq!(*encoded.last().unwrap(), BINCODE_OPTION_NONE_TAG);
    }
    ...
    /// The MoE `PlainProof` tail encodes byte-exactly against Pearl's bincode-1
    /// serialization of `PlainProof { moe: Some(MoEProofParams) }` and strictly
    /// deserializes under the same layout.
    #[test]
    fn pearl_moe_share_serializes_byte_exactly_and_round_trips() {
        ...
        // The MoE option tag replaces the dense `Option::None` tag.
        let dense_len = {
            let mut d = build_dense_proof();
            d.moe = None;
            let mut v = Vec::new();
            d.encode_bincode1(&mut v).unwrap();
            v.len()
        };
        assert_eq!(manual[dense_len - 1], BINCODE_OPTION_SOME_TAG);
