...
//          sizes. However, the implementations of these functions in nockvm_crypto
...
pub fn jet_siva_en(context: &mut Context, subject: Noun) -> Result {
    ...
    let txt = slot(subject, 6, &space)?.as_atom()?;
    let key = slot(subject, 60, &space)?.as_atom()?;
    let ads = slot(subject, 61, &space)?;

    if met(3, key, &space) > 32 {
        Err(JetErr::Punt)
    } else {
...
pub fn jet_siva_de(context: &mut Context, subject: Noun) -> Result {
    let stack = &mut context.stack;
    let space = stack.noun_space();
    let iv = slot(subject, 12, &space)?.as_atom()?;
    let len = slot(subject, 26, &space)?.as_atom()?;
    let txt = slot(subject, 27, &space)?.as_atom()?;
    let key = slot(subject, 60, &space)?.as_atom()?;
    let ads = slot(subject, 61, &space)?;
...
mod util {
    ...
    use crate::noun::{Atom, IndirectAtom, Noun, NounSpace, D, T};
    ...
    fn _allocate_ads(
        ...
        let length = list::util::lent(ads, space)?;
    ...
    pub(super) fn _siv_en<const N: usize>(
        stack: &mut NockStack,
        key: &mut [u8; N],
        ads: Noun,
        txt: Atom,
        ...
        unsafe {
            ...
            match txt_len {
                0 => {
                    ac_aes_siv_en::<N>(key, &mut [], siv_data, iv_bytes, &mut [0u8; 0])
                        .unwrap_or_else(|err| {
                            panic!(
                    ...
                    Ok(T(
                        stack,
                        &[iv.normalize_as_atom(space).as_noun(), D(0), D(0)],
                    ))
                }
                _ => {
                    ...
                    Ok(T(
                        stack,
                        &[
                            iv.normalize_as_atom(space).as_noun(),
                            D(txt_len as u64),
                            out_atom.normalize_as_atom(space).as_noun(),
                        ],
                    ))
                }
            }
    ...
    pub(super) fn _siv_de<const N: usize>(
        stack: &mut NockStack,
        key: &mut [u8; N],
        ads: Noun,
        iv: Atom,
        ...
        unsafe {
            ...
            match txt_len {
                0 => {
                    ac_aes_siv_de::<N>(key, &mut [], siv_data, iv_bytes, &mut [0u8; 0])
                        .unwrap_or_else(|err| {
                            panic!(
                                "Panicked with {err:?} at {}:{} (git sha: {:?})",
                                file!(),
            ...
            Ok(T(
                stack,
                &[D(0), out_atom.normalize_as_atom(space).as_noun()],
            ))
        }
    }
}
...
mod tests {
    ...
    pub(super) fn assert_jet_in_door(
        ...
        // std::io::stderr().flush().unwrap_or_else(|| panic!("Panicked at {}:{} (git sha: {:?})", file!(), line!(), option_env!("GIT_SHA")));
        let jet_res = jet(c, sbj).unwrap_or_else(|err| {
            panic!(
                "Panicked with {err:?} at {}:{} (git sha: {:?})",
                file!(),
                line!(),
